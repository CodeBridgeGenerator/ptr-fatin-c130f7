...
import dashboard1FakerFactory from "./dashboard1FakerFactory";
...
const Dashboard1FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dashboard1FakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dashboard1").create(elem));
...
export default connect(mapState, mapDispatch)(Dashboard1FakerDialogComponent);
