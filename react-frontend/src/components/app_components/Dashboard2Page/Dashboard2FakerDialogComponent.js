...
import dashboard2FakerFactory from "./dashboard2FakerFactory";
...
const Dashboard2FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dashboard2FakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dashboard2").create(elem));
...
export default connect(mapState, mapDispatch)(Dashboard2FakerDialogComponent);
